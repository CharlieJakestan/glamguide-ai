...
export const initAdvancedFaceDetection = async (): Promise<boolean> => {
  ...
  console.log('Starting initAdvancedFaceDetection...');
  ...
  loadingPromise = new Promise(async (resolve) => {
    try {
      ...
      try {
        ...
        // Try fallback CDN
        try {
          console.log('Attempting to load face detection models from fallback:', FALLBACK_MODEL_URL);
...
export const detectFaces = async (
  ...
  if (!video || !modelsLoaded) {
    console.log('Cannot detect faces: video element not available or models not loaded');
  ...
  try {
    ...
    console.log('Face detection completed, found faces:', detections.length);
    ...
    if (results && results.length > 0) {
      ...
      console.log('Face detection successful, returning results');
    } else {
